...
def iteration(distance_matrix, projection, learning_rate, n_components):
    size = len(projection)
    error = 0
...
class FastForceScheme:
    ...
    def _fit(self, X, y, metric):
        ...
        for k in range(self.max_it_):
            ...
            new_error = iteration(distance_matrix, self.embedding_, learning_rate, self.n_components_)
