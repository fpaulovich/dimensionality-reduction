...
def move2D(ins1, distance_matrix, intervals, projection, learning_rate):
    size = len(projection)
    ...
    for ins2 in prange(size):
        if ins1 != ins2:
            ...
            new_x = projection[ins2][0] + learning_rate * delta * (x1x2 / dr2)
            min_int = intervals[ins2][0]
            max_int = intervals[ins2][1]
            projection[ins2][0] = min_int if new_x < min_int else (max_int if new_x > max_int else new_x)

            # if new_x < min_int:
            #     projection[ins2][0] = min_int
            #     delta = (min_int - projection[ins2][0]) / (x1x2 / dr2)
            # elif new_x > max_int:
            #     projection[ins2][0] = max_int
            #     delta = (max_int - projection[ins2][0]) / (x1x2 / dr2)
            # else:
            #     projection[ins2][0] = new_x

            # y-coordinate move freely
...
@njit(parallel=True, fastmath=False)
def move(ins1, distance_matrix, intervals, projection, learning_rate):
    # size = len(projection)
    # total = len(distance_matrix)
    # error = 0
    #
...
@njit(parallel=False, fastmath=False)
def iteration(index, distance_matrix, intervals, projection, learning_rate, n_components):
    size = len(projection)
    error = 0
    ...
    for i in range(size):
        ...
        if n_components == 2:
            error += move2D(ins1, distance_matrix, intervals, projection, learning_rate)
        else:
            error += move(ins1, distance_matrix, intervals, projection, learning_rate)

    return error / size


# calculate adaptively intervals according to the features values
def compute_intervals(fixed_feature, alpha):
    ...
    final_intervals = np.zeros((len(fixed_feature), 2), dtype=np.float32)

    for i in range(len(fixed_feature)):
        final_intervals[i][0] = intervals_dict[fixed_feature[i]][0]
        final_intervals[i][1] = intervals_dict[fixed_feature[i]][1]
...
class DimenFixForceScheme:
    ...
    def _fit(self, X, y, distance_function):
        ...
        intervals = compute_intervals(self.fixed_feature_, self.alpha_)
        ...
        for k in range(self.max_it_):
            ...
            new_error = iteration(index, distance_matrix, intervals, self.embedding_, learning_rate, self.n_components_)
